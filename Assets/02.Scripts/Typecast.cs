...
{
    private const string VOICE_ID = "666a9871abcf27a5169850d0";
    ...
    private IEnumerator RequestSpeakCoroutine(string text, TaskCompletionSource<AudioClip> tcs)
    {
        ...
        using (var postReq = new UnityWebRequest("https://typecast.ai/api/speak", "POST"))
        {
            ...
            postReq.SetRequestHeader("Authorization", $"Bearer {APIKeys.TTSAPI_KEY}");
            ...
            while (audioUrl == null)
            {
                ...
                {
                    getReq.SetRequestHeader("Authorization", $"Bearer {APIKeys.TTSAPI_KEY}");
